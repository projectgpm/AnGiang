...
namespace QLCafe.DTO
{
    public class DTO_DangNhap
    {
        ...
        public DTO_DangNhap(DataRow dr)
        {
            Id = Int32.Parse(dr["ID"].ToString());
            Tendangnhap = dr["TenDangNhap"].ToString();
            Idchinhanh = dr["IDChiNhanh"].ToString();
            Manhanvien = dr["MaNhanVien"].ToString();
            Tennguoidung = dr["TenNguoiDung"].ToString();
            Sdt = dr["SDT"].ToString();
            IDNhomNguoiDung = Int32.Parse(dr["IDNhomNguoiDung"].ToString()); 
        }
    }
}
