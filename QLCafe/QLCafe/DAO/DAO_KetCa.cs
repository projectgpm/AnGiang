...
namespace QLCafe.DAO
{
    public class DAO_KetCa
    {
        ...
        {
            string sTruyVan = string.Format(@"SELECT GioRa FROM [CF_HoaDon] WHERE IDNhanVien = {0} AND TrangThai = 1 AND TrangThaiKetCa = 0 ORDER BY ID ASC", IDNhanVien);
            DataTable data = new DataTable();
            data = DataProvider.TruyVanLayDuLieu(sTruyVan);
            if (data.Rows.Count > 0)
            {
        ...
        {
            string sTruyVan = string.Format(@"INSERT INTO CF_KetCa(ThoiGianBatDau,ThoiGianKetThuc,TongTienSauCa,IDNhanVien,IDChiNhanh,TongTien,GiamGia) VALUES (CAST('{0}' AS DateTime),getdate(),'{1}','{2}','{3}','{4}','{5}')", GioVao.Substring(0,GioVao.LastIndexOf(" ")), TongTienSauCa.ToString(), IDNhanVien, IDChiNhanh, TongTien.ToString(), GiamGia.ToString());
            return DataProvider.TruyVanKhongLayDuLieu(sTruyVan);
        }
    }
}
